...
const useApi = <TQueryKey extends QueryKey = QueryKey, T = unknown>(
  queryKey: TQueryKey,
  queryFn: (ctx: QueryFunctionContext<TQueryKey>, token: string) => Promise<T>
) => {
...
const query = useApi(["users"], async (ctx, token) => {
  type tests = [Expect<Equal<typeof ctx.queryKey, string[]>>, Expect<Equal<typeof token, string>>];
...
type tests = [Expect<Equal<typeof query.data, { id: number; name: string }[] | undefined>>];

// If you pass in an array of numbers in the queryKey, the type of ctx.queryKey
// should be number[]
useApi([1, 2], async (ctx, token) => {
  type tests = [Expect<Equal<typeof ctx.queryKey, number[]>>, Expect<Equal<typeof token, string>>];
});
